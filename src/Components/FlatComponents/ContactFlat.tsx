...
const ContactFlat: FC<Props> = (props) => {
    const token = localStorage.getItem(LS_AUTH_TOKEN);
    console.log("Token ", token)
    const [flatResponse, setResponse] = useState<any>();


    useEffect(() => {
        async function fetchData() {
            const url = BASE_URL + "/flat-creator";
            try {
                const flatResponse = await axios.get(url, { headers: { 'Content-Type': 'application/json', 'Authorization': token! }, params: { flat_id: props.flat_id } });
                setResponse(flatResponse.data)
            } catch (error) {
                ...
                <div className="p-5 w-full max-w-lg border-0 rounded-lg shadow-lg bg-white  ">
                    ...
                    <div className="border-2  w-full  h-96 rounded-lg p-6 shadow-2xl pt-2 ">
                        ...
                        <div className="flex space-x-7 pt-3">
                            ...
                            <div>
                                <h1 className="font-semibold text-gray-800">{flatResponse?.firstName} {flatResponse?.lastName}</h1>
...
ContactFlat.defaultProps = {
}
export default memo(ContactFlat);
export { };